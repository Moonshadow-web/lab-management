...
QC_GOAL_EXACT_OVERRIDES: dict[str, str] = {
    ...
    "CER": "8%",
    "P2PSA": "7.5%",
    # CK-MB 肌酸激酶同工酶——避免被「肌酸激酶(CK)」cv=5.5% 前缀截胡
    "肌酸激酶同工酶": "10%",
    "肌酸激酶同工酶CKMB": "10%",
    "CK-MB": "10%",
    "CKMB": "10%",
    "ck-mb": "10%",
    "ckmb": "10%",
    # 用户指定质量目标（2026-08-06，室内质控月结「自动」列）：
    #   乙肝表面抗原(HBsAg) / TPAb(梅毒螺旋体抗体) 允许不精密度 = 10%，
    #   甲状腺球蛋白(Tg) = 8%。
    # 这些项在 quality_requirements 表里 cv 为空、tea 为「与预期结果一致」或 NCCL tea/3，
    # 旧逻辑只能落到默认 10%（Tg 本应按 wst403 cv=8.0% 取 8%，但按缩写 Tg/TG 上传时
    # 因与「甘油三酯(TG)」撞名无法精确命中 → 也落默认 10%）。此处精确钉死目标值。
    # 注：'tg' 同时覆盖 Tg/TG（_norm 大小写归一）；若贵科甘油三酯 LIS 名恰为字面 "TG"，
    # 会被一并判为 8%，请改用中文「甘油三酯」上传（甘油三酯(TG) 本身 wst403 cv=5.0%）。
    "表面抗原": "10%",
    "乙肝表面抗原": "10%",
    "乙型肝炎病毒表面抗原": "10%",
    "HBsAg": "10%",
...
_LIS_ITEM_ALIASES = {
    ...
    # 2026-07-28 心肌/免疫/特殊项目
    # CK-MB：LIS 导出常见三种写法，统一归到中间桥接名「肌酸激酶同工酶」（seed 已建该条 cv=10%）
    "ck-mb": "肌酸激酶同工酶",
    "ckmb": "肌酸激酶同工酶",
    "ck-mbmass": "肌酸激酶-MB(μg/L)",  # 质量法（μg/L）写法
    "ckmbmass": "肌酸激酶-MB(μg/L)",
    "p2sa": "P2PSA",
    "肌钙蛋白i": "超敏肌钙蛋白Ⅰ",
    "肌钙蛋白ⅰ": "超敏肌钙蛋白Ⅰ",  # LIS 可能用全角罗马数字 Ⅰ
    "tpo抗体": "抗甲状腺过氧化物酶抗体",
    "甲球抗体": "抗甲状腺球蛋白抗体",
    "内因子抗体测定": "抗内因子抗体",
    "stfr": "可溶性转铁蛋白受体",
