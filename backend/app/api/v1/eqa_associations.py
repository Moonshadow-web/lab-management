...
# ---------------------------------------------------------------------------
# 核对端点：有卫健委 EQA 计划、但卫健委标准源(nccl-2026)缺标准的项目
# 基于系统真实关联（_compute_associations 算出的 has_wjw）+ 标准源匹配
# ---------------------------------------------------------------------------
def _norm_qr(s: str) -> str:
    return re.sub(r"[\s\-（）().]", "", s or "").lower()


def _find_nccl(name: str, nccl_map: dict) -> Optional[QualityRequirement]:
    """把项目库名匹配到卫健委标准源(nccl-2026)的具体条目。"""
    # 1 精确
    if name in nccl_map:
        return nccl_map[name]
    # 2 归一化精确
    n = _norm_qr(name)
    for k, v in nccl_map.items():
        if _norm_qr(k) == n:
            return v
    # 3 包含（双向）
    for k, v in nccl_map.items():
        if name in k or k in name:
            return v
    # 4 同义词：二氧化碳 ↔ 碳酸氢根 等
    syn = {"二氧化碳": ["碳酸氢根", "HCO3", "Bicarbonate", "总二氧化碳", "二氧化碳结合力", "CO2"]}
    for s in syn.get(name, []):
        if s in nccl_map:
            return nccl_map[s]
        for k, v in nccl_map.items():
            if s in k or k in s:
...
@router.get("/_meta/wjw-gap", summary="核对：有卫健委EQA计划但缺卫健委标准的项目")
def check_wjw_standard_gap(
    db: Session = Depends(get_db),
    _=Depends(get_current_user),
):
    """返回「系统已关联到卫健委EQA计划(has_wjw) 但 卫健委标准源(nccl-2026)查不到标准」的项目清单。

    用于核对：项目库里标了卫健委的，是否都能在卫健委EQA标准里找到——找不到的需补录。
    """
    assoc = _compute_associations(db)  # 系统真实关联
    nccl_map = {
        r.item_name: r
        for r in db.query(QualityRequirement).filter(QualityRequirement.source == "nccl-2026").all()
    }
    gap = []
    total_wjw = 0
    for item in assoc:
        if not item.has_wjw:
            continue
        total_wjw += 1
        if not _find_nccl(item.name, nccl_map):
            gap.append({
                "id": item.id,
                "name": item.name,
...
def _reset_item_eqa_cache(db: Session):
    """重新计算所有 test_items 的 has_eqa 缓存。"""
    assoc_items = _compute_associations(db)
    for r in assoc_items:
        db.query(TestItem).filter(TestItem.id == r.id).update({"has_eqa": r.has_eqa})
    db.commit()
