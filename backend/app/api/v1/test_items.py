...
# 临床最常用项目 TOP 档（精确名匹配，最常做的"急诊/组合"项目，避免子串误伤如"钠"伤及"钠尿肽"）
_TOP_RANK = {
    "钾", "钠", "氯", "钙", "镁", "磷", "二氧化碳", "葡萄糖",
    "尿素", "肌酐", "尿酸", "胱抑素C", "同型半胱氨酸",
    "丙氨酸氨基转移酶", "天冬氨酸氨基转移酶", "γ-谷氨酰基转移酶", "碱性磷酸酶",
    "乳酸脱氢酶", "总蛋白", "白蛋白", "前白蛋白",
    "总胆红素", "直接胆红素", "总胆汁酸", "胆碱酯酶",
    "甘油三酯", "总胆固醇", "高密度脂蛋白胆固醇", "低密度脂蛋白胆固醇",
    "淀粉酶", "脂肪酶", "肌酸激酶", "C反应蛋白", "糖化血红蛋白",
    "转铁蛋白",
}
# 其他常用项目（子串匹配）：不含"钠/钾/钙"等单字以免误伤
_COMMON_SUBSTR = [
    # 生化
...
_COMMON_CASE = case(
    (TestItem.name.in_(_TOP_RANK), 0),
    (or_(*[TestItem.name.ilike(f"%{kw}%") for kw in _COMMON_SUBSTR]), 1),
    else_=2,
)
