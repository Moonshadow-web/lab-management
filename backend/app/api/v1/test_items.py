...
# 默认排序：常见项目分类在前（生化→免疫→凝血→其他）；同类内临床常用项目在前，其余排后；最后按项目名升序
_CATEGORY_RANK = case(
...
# 临床最常用项目 TOP 档（精确名+显式排序序号，按急诊/组合必做项目顺序）
_TOP_RANK_ORDER = {
    # 电解质
    "钾": 0, "钠": 1, "氯": 2, "钙": 3, "镁": 4, "磷": 5, "二氧化碳": 6,
    # 血糖
    "葡萄糖": 7,
    # 肾功能
    "尿素": 8, "肌酐": 9, "尿酸": 10, "胱抑素C": 11, "同型半胱氨酸": 12,
    # 肝功能
    "丙氨酸氨基转移酶": 13, "天冬氨酸氨基转移酶": 14, "γ-谷氨酰基转移酶": 15,
    "碱性磷酸酶": 16, "乳酸脱氢酶": 17, "总胆红素": 18, "直接胆红素": 19,
    "总胆汁酸": 20, "胆碱酯酶": 21, "总蛋白": 22, "白蛋白": 23, "前白蛋白": 24,
    # 血脂
    "总胆固醇": 25, "甘油三酯": 26, "高密度脂蛋白胆固醇": 27, "低密度脂蛋白胆固醇": 28,
    # 心肌酶
    "肌酸激酶": 29,
    # 胰腺
    "淀粉酶": 30, "脂肪酶": 31,
    # 炎症/糖化/铁代谢
    "C反应蛋白": 32, "糖化血红蛋白": 33, "转铁蛋白": 34,
}
# 其他常用项目（子串匹配）：不含"钠/钾/钙"等单字以免误伤（BNP/糖化白蛋白/同型半胱氨酸→胱抑素C已在TOP）
_COMMON_SUBSTR = [
    # 生化
    "B型钠尿肽", "肌酸激酶同工酶", "羟丁酸", "腺苷脱氨酶", "载脂蛋白", "脂蛋白a",
    "小而密低密度", "铁蛋白", "β2-微球蛋白",
    # 免疫
...
# 排序档：TOP(0-34) → 常规(100) → 其他(200)
_common_whens = [(TestItem.name == k, v) for k, v in _TOP_RANK_ORDER.items()]
_common_whens.append(
    (or_(*[TestItem.name.ilike(f"%{kw}%") for kw in _COMMON_SUBSTR]), 100)
)
_COMMON_CASE = case(*_common_whens, else_=200)
