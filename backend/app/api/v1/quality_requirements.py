...
_SYNONYMS = {
    ...
    # 项目库名 → 标准源中的同义叫法（打通综合比对匹配）
    "总钙": ["钙(Ca)"],
    "无机磷": ["磷(P)", "磷酸根离子"],
    "B型钠尿肽": ["脑钠肽(BNP)", "BNP"],
    "超敏肌钙蛋白Ⅰ": ["肌钙蛋白I(TnI)", "肌钙蛋白-I"],
    "促甲状腺激素": ["促甲状腺素(TSH)", "促甲状腺刺激激素", "TSH"],
    "总胆汁酸": ["胆汁酸(BA)"],
    "尿微量白蛋白": ["微量白蛋白(尿液)"],
    "尿转铁蛋白": ["尿转铁蛋白(UTRF)"],
    # 项目库名（与标准源写法差异）↔ 标准源同义叫法（打通综合比对匹配）
    "白介素-6": ["白细胞介素-6(IL-6)"],
    "血浆D-二聚体": ["D-二聚体(D-Dimer)"],
    "不饱和铁结合力/总铁结合力": ["不饱和铁结合力(UIBC)", "总铁结合力(TIBC)"],
    # 糖化白蛋白(GA) 与糖化血清蛋白为同一分析物（GA 由糖化血清蛋白归一化），卫健委 EQA 靶值 ±20%
    "糖化白蛋白": ["糖化血清蛋白"],
    # CKMB 在项目库叫「肌酸激酶同工酶」；行标 WS/T 403 精确条目是带单位的两个名称，
    # 必须写全称，不能简写「肌酸激酶-MB」（否则会被「肌酸激酶」误匹配为同一项）。
    "肌酸激酶同工酶": ["肌酸激酶-MB(μg/L)", "肌酸激酶-MB(U/L)", "CK-MB"],
}
