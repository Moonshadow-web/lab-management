...
- **文件存储 = 腾讯云 COS 云存储（根治 MySQL 内存告警）**：v1 是 MySQL LONGBLOB（documents.data/document_versions.data），跑起来就触发 TDSQL-C 内存 90% 告警；v2 已改为腾讯云 COS 对象存储。documents/document_versions 两表加 `cloud_key: String(500)` 字段（存 COS 对象键 `{module}/{uuid8}_{safe_name}`，如 `documents/2a9da776_xxx.docx`）。上传/新版本先调 `cos_storage.save()` 上传到 COS（key 落库、data 置 NULL），COS 失败兜底仍写 BLOB+磁盘；下载优先用 COS 临时签名 URL **重定向 302**（不经过服务器中转，省带宽），预览/无 COS key 时 `cos_storage.get_bytes()` 拉字节；三级回退 COS→MySQL BLOB→磁盘（与 ComparisonAttachment/InterlabAttachment 一致）。`cos_storage.py` 封装 `qcloud-cos-python-sdk-v5`，凭据走环境变量 `COS_SECRET_ID/COS_SECRET_KEY/COS_BUCKET/COS_REGION`（凭据不进 git，通过 CloudBase 控制台注入；bucket=`636c-cloud1-0gjhamv53ff2298d-1408547492` region=`ap-shanghai`）。**关键踩坑**：`put_object` 的 `ContentLength` 必须是 **str** 不是 int（SDK 校验 header 必须是 str/bytes，否则 `CosClientError: must be of type str or bytes, not <class 'int'>`）。Dockerfile 加 `gcc python3-dev` 编译 `crcmod` C 扩展（pip 在 slim 镜像里需编译 crcmod）。迁移端点 `_migrate_to_cos`（admin，幂等）已跑通：462 docs + 413 versions = 875 个 BLOB 全部进 COS，documents_with_data=0 / versions_with_data=0 / total_blob_mb=0；分批 yield_per(20) + 失败单条跳过，避免一次加载全部 BLOB 撑爆连接。CFS 持久卷早因 bug 弃用，COS 才是长期方案。仅**早期(pre-迁移)上传、字节已随旧容器丢失**的 ~12 个文档需重传（doc 503 BG-SM-PX-005 / 504 / 506 / 236 / 163 / 502 / 505 / 394 / 395 / 396 / 397 / 399；另 14 条是 file_path 空记录非丢失）。**ComparisonAttachment/InterlabAttachment 同病同治**：后续可复用 cos_storage.py 迁移，根治全平台 BLOB 内存占用。
