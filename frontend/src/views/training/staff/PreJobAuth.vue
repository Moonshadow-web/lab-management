...
META.forEach((p) => { examData.value[p.name] = { qaNotes: {}, practicalScores: {}, theoryAnswers: {}, qaResult: '合格', mastery: '基本了解' } })
...
watch(positions, () => {
  ...
  positions.value.forEach((p) => { if (!examData.value[p]) examData.value[p] = { qaNotes: {}, practicalScores: {}, theoryAnswers: {}, qaResult: '合格', mastery: '基本了解' } })
}, { flush: 'sync' })
...
function printForm(row) {
  ...
  const examRows = Object.entries(row.exam_json || {}).map(([post, d]) =>
    `<tr><td style="text-align:center;">${esc(post)}</td><td style="text-align:center;">${esc(d.qaResult || '')}</td><td style="text-align:center;">${esc(d.mastery || '')}</td></tr>`).join('')
  const html = `
  ...
  <table style="border:1.5px solid #333;font-size:13px;">
    ...
    <tr><td style="text-align:center;background:#f7f7f7;">考核岗位</td><td colspan="3" style="padding:6px 10px;">${esc((row.positions_json || []).join('、'))}</td></tr>
    <tr><td style="text-align:center;background:#f7f7f7;">授权权限</td><td colspan="3" style="padding:6px 10px;">${esc(perm)}</td></tr>
  </table>
  <h3>一、考核仪器</h3>
  <table style="border-collapse:collapse;width:100%;font-size:12px;">
    <tr><th style="border:1px solid #333;background:#f1f5f9;padding:5px;">岗位</th><th style="border:1px solid #333;background:#f1f5f9;padding:5px;">仪器</th><th style="border:1px solid #333;background:#f1f5f9;padding:5px;">仪器编号</th><th style="border:1px solid #333;background:#f1f5f9;padding:5px;width:26%;">关联检验项目</th><th style="border:1px solid #333;background:#f1f5f9;padding:5px;width:10%;">结果</th></tr>
    ${inst}
  </table>
  <h3>二、分岗位考核记录</h3>
  <table style="border-collapse:collapse;width:100%;font-size:12px;">
    <tr><th style="border:1px solid #333;background:#f1f5f9;padding:5px;">岗位</th><th style="border:1px solid #333;background:#f1f5f9;padding:5px;">口头问答</th><th style="border:1px solid #333;background:#f1f5f9;padding:5px;">掌握程度</th></tr>
    ${examRows}
  </table>
  <h3>三、考核意见</h3>
  <table style="border:1.5px solid #333;font-size:13px;">
