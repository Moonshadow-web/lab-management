        ...
        <div style="flex:1;min-width:0">
          ...
          <template v-if="category === '定量'">
            ...
            <el-table :data="quantDisplay" size="small" border max-height="460">
              ...
              <el-table-column label="参照值" width="140">
                <template #header>
                  <span>参照值（靶机）</span>
                </template>
                <template #default="{ row }">
                  <div style="display:flex;flex-direction:column;gap:2px">
                    ...
                    <span style="font-size:10px;color:#888">{{ rowEffRefName(row) }}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column v-for="ins in compared" :key="ins.id" :label="ins.name">
                ...
                <template #default="{ row }">
                  <div v-if="!isApplicable(row.item, ins.id)">
                    <!-- 动态靶机：该比对仪器是当前项目的实际参照 -->
                    <div v-if="rowEffRefName(row) === ins.name" class="tag-ref">参照</div>
                    <div v-else class="masked" title="该项目不在此仪器上开展">/</div>
                  </div>
...
const globalRefId = ref(0)

const isDone = (name) => savedItems.value.has(name)

// 计算每项目的有效靶机：若组参照仪不适用，用第一个可用的比对仪器替代
function effRefFor(item) {
  const meta = itemMeta.value[item]
  if (!meta) return globalRefId.value
  const ids = (meta.instrument_ids || []).map(Number)
  if (!ids.length) return globalRefId.value  // 空=全部适用，用组靶机
  if (ids.includes(globalRefId.value)) return globalRefId.value
  // 找第一个在用且属于本组的可比较仪器
  const instIds = allInstruments.value.filter((i) => !i.is_reference).map((i) => i.id)
  const found = instIds.find((iid) => ids.includes(iid))
  return found || globalRefId.value
}
function effRefName(item) {
  const id = effRefFor(item)
...
function isApplicable(item, insId) {
  ...
  const ids = meta.instrument_ids || []
  // 如果该项目有动态靶机，且该仪器恰好是动态靶机 → 不在可编辑区显示（其值在参照值列录入）
  if (ids.length && effRefFor(item) !== globalRefId.value && Number(insId) === effRefFor(item)) return false
  // 原有遮蔽逻辑
  if (!ids.length) return true
  return ids.includes(Number(insId))
}
// 用于参照值列头显示：当前行的实际靶机
function rowEffRefName(row) {
  return effRefName(row.item)
}
...
async function load() {
  ...
  try {
    ...
    globalRefId.value = data.global_ref_id || 0
    itemMeta.value = {}
...
.tag-ref {
  display: inline-block; text-align: center; color: #409eff; font-size: 12px; font-weight: 600;
  background: #ecf5ff; border-radius: 4px; padding: 6px 12px; user-select: none;
}
.nav-item {
